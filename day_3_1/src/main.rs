...
fn main() {
    ...
    for (line_index, line) in lines.iter().enumerate() {
        ...
        let mut pos = 0;
        while let Some(first_digit_displacement) = it.position(|symbol| symbol.is_ascii_digit()) {
            let mut is_part_number = false;
            let first_digit_index = pos + first_digit_displacement;
            pos = first_digit_index + 1;
            let (last_digit_index, end) = if let Some(first_non_digit_displacement) =
                it.position(|symbol| !symbol.is_ascii_digit())
            {
                let first_non_digit_index = pos + first_non_digit_displacement;
                pos = first_non_digit_index + 1;
                is_part_number |= line.chars().nth(first_non_digit_index) != Some('.');
                (first_non_digit_index - 1, first_non_digit_index)
            } else {
                (line.len() - 1, line.len() - 1)
            };
            let start = if first_digit_index > 0 {
                is_part_number |= line.chars().nth(first_digit_index - 1) != Some('.');
                first_digit_index - 1
            } else {
                first_digit_index
            };
            is_part_number |= (line_index > 0
                && lines[line_index - 1][start..=end]
                    .chars()
                    .any(|symbol| symbol != '.'))
                || (line_index < lines.len() - 1
                    && lines[line_index + 1][start..=end]
                        ...
                        .any(|symbol| symbol != '.'));
            if is_part_number {
                result += line[first_digit_index..=last_digit_index]
                    .parse::<usize>()
                    .unwrap_or(0);
            }
        }
