...
fn main() {
    ...
    let mut breadcrumbs = vec![vec![false; map.len()]; map_len_y as usize];
    let mut steps = 0;
    let mut positions = vec![
        (Left, start_y, start_x - 1),
        (Right, start_y, start_x + 1),
        (Up, start_y - 1, start_x),
        (Down, start_y + 1, start_x),
    ];
    while !positions.is_empty() {
        // Check correctness of all positions
        for i in (0..positions.len()).rev() {
            ...
            if let Some(new_pos) = match (map[y][x], *pos) {
                ('|', (Up, y, x)) | ('L', (Left, y, x)) | ('J', (Right, y, x)) => follow(Up, y, x),
                ('|', (Down, y, x)) | ('F', (Left, y, x)) | ('7', (Right, y, x)) => {
                    follow(Down, y, x)
                }
                ('7', (Up, y, x)) | ('J', (Down, y, x)) | ('-', (Left, y, x)) => follow(Left, y, x),
