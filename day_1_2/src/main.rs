const INPUT: &str = "threerznlrhtkjp23mtflmbrzq395three
9sevenvlttm
3twochzbv
    ...
    let result: u32 = INPUT
        ...
        .map(|line| {
            let mut first_digit = line
                .chars()
                .enumerate()
                .find_map(|symbol| Some((symbol.0, symbol.1.to_digit(10)?)))
                .unwrap_or((line.len(), 0));
            let mut last_digit = line
                .chars()
                .rev()
                .enumerate()
                .find_map(|symbol| Some((line.len() - symbol.0, symbol.1.to_digit(10)?)))
                .unwrap_or((0, 0));
            for (key, value) in &spelled {
                ...
                if let Some(index) = line.rfind(key) {
                    if index + 1 > last_digit.0 {
                        last_digit.0 = index + 1;
                        last_digit.1 = *value;
                    }
                }
            }
