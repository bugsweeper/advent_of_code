...
const INPUT: &str = "Game 1: 10 red, 7 green, 3 blue; 5 blue, 3 red, 10 green; 4 blue, 14 green, 7 red; 1 red, 11 green; 6 blue, 17 green, 15 red; 18 green, 7 red, 5 blue
    ...
    let result: u32 = INPUT
        ...
        .filter_map(|line| {
            ...
            let id = games
                ...
                .parse::<u32>()
                .ok()?;
            if games.next()?.split(';').all(|game| {
                game.split(',').all(|cube| {
                    let mut parts = cube.trim().split_ascii_whitespace();
                    parts.next().unwrap().parse::<u32>().unwrap() <= colors[parts.next().unwrap()]
                })
            }) {
                Some(id)
            } else {
