const INPUT: &str = "F7.-.|.-F777F7.7-|F-7-7J.-J-7-JFFFJ-F--7FJ-|7F-JF|-F.|F-7-F7-L-.FF-|7.--FL-L7-J-FJF-F7F--77..F|.F--7F777|-JJ7FJF---J-L7L-F-LJ7F.|F7JJ-777.-|
...
enum State {
    Outside,
    Inside,
    HorizontalFromBottomInside,
    HorizontalFromTopInside,
    HorizontalFromBottomOutside,
    HorizontalFromTopOutside,
}

use std::collections::HashSet;
use Direction::{Down, Left, Right, Up};
use State::{
    HorizontalFromBottomInside, HorizontalFromBottomOutside, HorizontalFromTopInside,
    HorizontalFromTopOutside, Inside, Outside,
};

fn follow(direction: Direction, y: isize, x: isize) -> Option<(Direction, isize, isize)> {
    match direction {
        Up => Some((Up, y - 1, x)),
        Down => Some((Down, y + 1, x)),
        Left => Some((Left, y, x - 1)),
        Right => Some((Right, y, x + 1)),
    }
}
fn main() {
    ...
    let mut map = INPUT
        ...
        .map(|(index, line)| {
            ...
            if let Some(column) = line.iter().position(|&symbol| symbol == 'S') {
                (start_y, start_x) = (index as isize, column as isize);
            }
            line
        })
        .collect::<Vec<_>>();
    let map_len_y = map.len() as isize;
    let map_len_x = if map_len_y == 0 {
        0
    } else {
        map[0].len() as isize
    };
    let mut breadcrumbs = vec![vec![0; map_len_x as usize]; map.len()];
    let mut positions = [
        Some((Left, start_y, start_x - 1)),
        Some((Right, start_y, start_x + 1)),
        Some((Up, start_y - 1, start_x)),
        Some((Down, start_y + 1, start_x)),
    ];
    let posible_starting_symbols = [
        ['J', '-', '7'],
        ['L', '-', 'F'],
        ['J', '|', 'L'],
        ['7', '|', 'F'],
    ];
    let mut starting_directions = (0, 1);
    while positions.iter().any(Option::is_some) {
        // Check correctness of all positions and move to next position
        for i in 0..positions.len() {
            if let Some(ref mut pos) = positions[i] {
                if pos.1 < 0 || pos.1 >= map_len_y || pos.2 < 0 || pos.2 >= map_len_x {
                    positions[i] = None;
                    continue;
                }
                let (y, x) = (pos.1 as usize, pos.2 as usize);
                if let Some(new_pos) = match (map[y][x], *pos) {
                    ('|', (Up, y, x)) | ('L', (Left, y, x)) | ('J', (Right, y, x)) => {
                        follow(Up, y, x)
                    }
                    ('|', (Down, y, x)) | ('F', (Left, y, x)) | ('7', (Right, y, x)) => {
                        follow(Down, y, x)
                    }
                    ('7', (Up, y, x)) | ('J', (Down, y, x)) | ('-', (Left, y, x)) => {
                        follow(Left, y, x)
                    }
                    ('F', (Up, y, x)) | ('L', (Down, y, x)) | ('-', (Right, y, x)) => {
                        follow(Right, y, x)
                    }
                    _ => None,
                } {
                    if breadcrumbs[y][x] > 0 {
                        positions[i] = None;
                        if positions.iter().all(Option::is_none) {
                            starting_directions = (i, breadcrumbs[y][x] - 1);
                        }
                    } else {
                        breadcrumbs[y][x] = i + 1;
                        *pos = new_pos;
    ...
    let (direction1, direction2) = (
        HashSet::from(posible_starting_symbols[starting_directions.0]),
        HashSet::from(posible_starting_symbols[starting_directions.1]),
    );
    let (ustart_y, ustart_x) = (start_y as usize, start_x as usize);
    // Starting pos should be part of path to distinguish inside/outside correctly
    map[ustart_y][ustart_x] = *direction1.intersection(&direction2).next().unwrap();
    breadcrumbs[ustart_y][ustart_x] = starting_directions.0 + 1;
    let mut result = 0;
    for i in 0..map.len() {
        ...
        for j in 0..map[0].len() {
            state = match (
                map[i][j],
                state,
                breadcrumbs[i][j] == starting_directions.0 + 1
                    || breadcrumbs[i][j] == starting_directions.1 + 1,
            ) {
                ('F', Inside, true) => HorizontalFromBottomInside,
                ('F', Outside, true) => HorizontalFromBottomOutside,
                ('L', Inside, true) => HorizontalFromTopInside,
                ('L', Outside, true) => HorizontalFromTopOutside,
                ('|', Outside, true)
                | ('J', HorizontalFromBottomOutside | HorizontalFromTopInside, true)
                | ('7', HorizontalFromTopOutside | HorizontalFromBottomInside, true) => Inside,
                ('|', Inside, true)
                | ('J', HorizontalFromBottomInside | HorizontalFromTopOutside, true)
                | ('7', HorizontalFromTopInside | HorizontalFromBottomOutside, true) => Outside,
                (_, Inside, false) => {
                    result += 1;
                    map[i][j] = 'I';
                    Inside
